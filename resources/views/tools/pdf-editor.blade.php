                ...
                <div class="w-fit min-w-full min-h-full flex flex-col items-center justify-start py-8 px-4 sm:px-8">
                    ...
                    <div id="pdfEditorOverlay"
                        ...
                        <div x-show="['pointer', 'edit-text', 'text'].includes(activeTool)" class="absolute inset-0 pointer-events-none z-10" x-cloak>
                            {{-- Master Content Outline (Dashed border around overall text region matching user screenshot) --}}
                            <template x-if="pageContentBox">
                                <div class="absolute pointer-events-auto cursor-pointer border-2 border-dashed border-sky-400/60 rounded-sm hover:border-sky-500 transition-all"
                                     :style="`left: ${pageContentBox.pctX}%; top: ${pageContentBox.pctY}%; width: ${pageContentBox.pctW}%; height: ${pageContentBox.pctH}%;`"
                                     @click.stop="handleMasterBoxClick($event)"
                                     title="พื้นที่ข้อความต้นฉบับ (คลิกที่บรรทัดใดก็ได้เพื่อสแกนและแก้ไขข้อความโดยคงรูปแบบ 100%)">
                                    <span class="absolute -top-5 right-1 text-[10px] font-semibold text-sky-600 bg-sky-50/90 px-1.5 py-0.5 rounded border border-sky-200 pointer-events-none">
                                        พื้นที่ข้อความต้นฉบับ (สแกนรูปแบบ 100%)
                                    </span>
                                </div>
                            </template>

                            {{-- Individual Detected Lines/Blocks (100% Original Font, Size, Align, Boldness) --}}
                            <template x-for="block in currentOriginalTextBlocks" :key="block.id">
                                <div class="absolute pointer-events-auto cursor-pointer border border-dashed border-sky-300/40 hover:border-brand-600 hover:bg-brand-500/10 hover:shadow-xs rounded-xs transition-all group"
                                     :style="`left: ${block.pctX}%; top: ${block.pctY}%; width: ${block.pctW}%; height: ${block.pctH}%;`"
                                     @click.stop="startEditingOriginalText(block)"
                                     :title="`คลิกเพื่อแก้ไขข้อความ: '${block.text}'`">
                                    <span class="hidden group-hover:flex absolute -top-6 left-0 bg-brand-600 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow-sm whitespace-nowrap items-center gap-1 z-30 pointer-events-none ring-1 ring-white/50">
                                        ...
                                        <span x-text="`${block.fontSize}pt ${block.bold ? 'หนา ' : ''}${block.align === 'center' ? '• กึ่งกลาง' : (block.align === 'right' ? '• ชิดขวา' : '')} • คลิกแก้ไข`"></span>
                                    </span>
                                </div>
                            </template>
                        </div>
