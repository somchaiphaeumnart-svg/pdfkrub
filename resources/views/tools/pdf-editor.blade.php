    ...
    <div class="bg-white rounded-2xl shadow-xl border border-slate-200/90 overflow-hidden flex flex-col h-[84vh] min-h-[700px]">
        ...
        <div class="bg-white border-b border-slate-200/80 px-3 py-1.5 flex flex-wrap items-center justify-between text-xs gap-2 shrink-0 select-none">
            ...
            <div x-show="activeRibbonTab === 'edit-content'" class="flex items-center gap-1.5 sm:gap-2 flex-wrap" x-cloak>
                ...
                <div class="flex items-center gap-0.5 bg-slate-100 p-0.5 rounded border border-slate-200" title="โหมดเลือกข้อความเดิม">
                    <button type="button" @click="setTextGroupMode('all')" :class="textGroupMode === 'all' ? 'bg-white shadow-2xs text-brand-600 font-bold' : 'text-slate-600 hover:text-slate-900'" class="px-2 py-0.5 rounded text-[11px] cursor-pointer">
                        ข้อความทั้งหมด
                    </button>
                    ...
                    <button type="button" @click="setTextGroupMode('line')" :class="textGroupMode === 'line' ? 'bg-white shadow-2xs text-brand-600 font-bold' : 'text-slate-600 hover:text-slate-900'" class="px-1.5 py-0.5 rounded text-[11px] cursor-pointer">
                        ทีละบรรทัด
                    </button>
                </div>
                    ...
                    <div id="pdfEditorOverlay"
                        ...
                        <div x-show="['pointer', 'edit-text', 'text'].includes(activeTool)" class="absolute inset-0 pointer-events-none z-10" x-cloak>
                            <template x-for="block in currentOriginalTextBlocks" :key="block.id">
                                <div class="absolute pointer-events-auto cursor-pointer border-2 border-dashed border-sky-400/80 bg-sky-500/5 hover:border-brand-600 hover:bg-brand-500/10 hover:shadow-md rounded transition-all group"
                                     :style="`left: ${block.pctX}%; top: ${block.pctY}%; width: ${block.pctW}%; height: ${block.pctH}%;`"
                                     @click.stop="startEditingOriginalText(block)"
                                     :title="block.isAllPage ? 'คลิกเพื่อแก้ไขข้อความทั้งหมดในหน้านี้' : `คลิกเพื่อแก้ไขข้อความนี้`">
                                    <span class="hidden group-hover:flex absolute -top-7 left-2 bg-brand-600 text-white text-[11px] font-bold px-2.5 py-1 rounded-md shadow-md whitespace-nowrap items-center gap-1.5 z-30 pointer-events-none ring-1 ring-white/60">
                                        <svg class="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m16.862 4.487 1.687-1.688a1.875 1.875 0 1 1 2.652 2.652L10.582 16.07a4.5 4.5 0 0 1-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 0 1 1.13-1.897l8.932-8.931Zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0 1 15.75 21H5.25A2.25 2.25 0 0 1 3 18.75V8.25A2.25 2.25 0 0 1 5.25 6H10"/></svg>
                                        <span x-text="block.isAllPage ? 'คลิกเพื่อแก้ไขข้อความทั้งหมด' : 'คลิกเพื่อแก้ไขข้อความ'"></span>
                                    </span>
                                </div>
                            </template>
                        </div>
                                ...
                                <template x-if="ann.type === 'text'">
                                    <div class="w-full h-full p-2 cursor-move rounded-xs transition-colors overflow-hidden flex flex-col"
                                        ...
                                        <textarea x-model="ann.text"
                                                  ...
                                                  :style="`text-align: ${ann.align || 'left'}; line-height: ${ann.lineHeight || 1.55}; font-size: inherit; font-family: inherit;`"
