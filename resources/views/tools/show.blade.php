...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
    ...
    <nav class="flex items-center gap-2 text-sm text-slate-500 mb-8">
        ...
        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="m8.25 4.5 7.5 7.5-7.5 7.5"/></svg>
        <span class="text-slate-300">{{ $tool['name_th'] ?? $tool['name'] }}</span>
    </nav>
    ...
    <div class="flex items-start gap-5 mb-10">
        ...
        <div>
            <div class="flex items-center gap-3 mb-1">
                <h1 class="text-3xl font-bold text-white">{{ $tool['name_th'] ?? $tool['name'] }}</h1>
                @if($tool['premium'])
            ...
            <p class="text-slate-400">{{ $tool['description_th'] ?? '' }}</p>
        </div>
    </div>
    ...
    <div class="glass rounded-2xl p-8 border border-accent-500/30 mb-8">
        <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
                ...
                <p class="text-slate-400 text-sm mb-4">อัปเกรดเพื่อใช้งาน {{ $tool['name_th'] }} ได้ไม่จำกัด พร้อม OCR ภาษาไทยความแม่นยำสูง</p>
    ...
    {{-- Upload Zone --}}
    <div class="glass rounded-3xl p-8 border border-white/[0.06]"
        ...
        <div class="upload-zone p-12 text-center cursor-pointer mb-6"
            ...
            <template x-if="!hasFiles">
                <div>
                    <div class="w-20 h-20 mx-auto mb-5 bg-brand-600/20 rounded-2xl flex items-center justify-center">
                    ...
                    <p class="text-white font-semibold text-lg mb-2">ลากและวางไฟล์ที่นี่</p>
                    <p class="text-slate-400 text-sm">รองรับ <span class="text-brand-400">{{ str_replace(',', ', ', $tool['accepts']) }}</span></p>
                    <p class="text-slate-500 text-xs mt-2">
                        ขนาดสูงสุด {{ auth()->check() && auth()->user()->getActivePlan()->max_file_size_mb >= 200 ? '200' : '10' }} MB
                        @if(!auth()->check()) · <a href="{{ route('register') }}" class="text-brand-400 hover:underline">สมัครสมาชิก</a>เพื่อเพิ่มขีดจำกัด @endif
                    </p>
                </div>
            </template>
            ...
            <template x-if="hasFiles">
                <div class="text-left w-full" @click.stop>
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <button @click="clearAll()" class="text-xs text-slate-400 hover:text-error-500 transition-colors px-3 py-1 rounded-lg hover:bg-error-500/10">ล้างทั้งหมด</button>
                    </div>
                    <div class="space-y-2 max-h-60 overflow-y-auto mb-2">
                        <template x-for="f in files" :key="f.id">
                            <div class="flex items-center gap-3 glass-light px-4 py-3 rounded-xl">
                                <svg class="w-5 h-5 text-brand-400 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M19.5 14.25v-2.625a3.375 3.375 0 0 0-3.375-3.375h-1.5A1.125 1.125 0 0 1 13.5 7.125v-1.5a3.375 3.375 0 0 0-3.375-3.375H8.25m2.25 0H5.625c-.621 0-1.125.504-1.125 1.125v17.25c0 .621.504 1.125 1.125 1.125h12.75c.621 0 1.125-.504 1.125-1.125V11.25a9 9 0 0 0-9-9Z" />
                                </svg>
                                <div class="flex-1 min-w-0">
                                    <p class="text-sm text-slate-200 truncate" x-text="f.name"></p>
                                    <p class="text-xs text-slate-500" x-text="f.sizeFormatted"></p>
                                </div>
                                <button @click="removeFile(f.id)" class="text-slate-500 hover:text-error-500 transition-colors p-1 rounded">
                                    <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18 18 6M6 6l12 12"/></svg>
                                </button>
                            </div>
                        </template>
                    </div>
                    <button @click="$refs.fileInput.click()" class="text-xs text-brand-400 hover:text-brand-300 transition-colors mt-2 flex items-center gap-1">
        ...
        <div class="mt-6">
            ...
            <div x-show="isProcessing" class="space-y-4">
                <div class="flex items-center justify-center gap-3">
                    ...
                    <span class="text-slate-300 font-medium" x-text="jobStatus === 'queued' ? 'รอในคิว...' : 'กำลังประมวลผล...'"></span>
                </div>
                ...
                <p class="text-center text-xs text-slate-500">ไฟล์จะถูกลบอัตโนมัติหลังดาวน์โหลด</p>
            </div>

            {{-- Done state: download button --}}
            <div x-show="isDone" class="text-center space-y-4">
                <div class="w-16 h-16 bg-success-500/20 rounded-full flex items-center justify-center mx-auto">
                    <svg class="w-8 h-8 text-success-500" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" d="m4.5 12.75 6 6 9-13.5"/>
                    </svg>
                </div>
                <p class="text-white font-semibold">ประมวลผลสำเร็จ! 🎉</p>
                <a href="#" :href="downloadUrl" :download="downloadFileName"
                   class="btn-primary px-8 py-3.5 rounded-xl inline-flex items-center gap-2">
                    ...
                    ดาวน์โหลด <span class="text-brand-200" x-text="downloadFileName"></span>
                </a>
                <p class="text-xs text-slate-500">
                    ขนาดไฟล์: <span x-text="downloadFileSize"></span>
                    &nbsp;·&nbsp;ลิงก์หมดอายุใน 60 นาที
                </p>
                <button @click="clearAll()" class="text-sm text-slate-400 hover:text-white transition-colors block mx-auto">
                    แปลงไฟล์อีกครั้ง →
                </button>
            </div>
            ...
            <div x-show="isFailed" class="text-center space-y-4">
                ...
                <p class="text-white font-semibold">เกิดข้อผิดพลาด</p>
    ...
    <div class="grid sm:grid-cols-3 gap-4 mt-8">
        <div class="glass rounded-xl p-4 border border-white/[0.05] text-center">
            ...
            <p class="text-xs text-slate-300 font-medium">ไฟล์ปลอดภัย</p>
            <p class="text-xs text-slate-500">เข้ารหัส AES-256</p>
        </div>
        <div class="glass rounded-xl p-4 border border-white/[0.05] text-center">
            <svg class="w-6 h-6 text-brand-400 mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M12 6v6h4.5m4.5 0a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/>
            </svg>
            <p class="text-xs text-slate-300 font-medium">ลบอัตโนมัติ</p>
            <p class="text-xs text-slate-500">ภายใน 48 ชั่วโมง</p>
        </div>
        <div class="glass rounded-xl p-4 border border-white/[0.05] text-center">
            <svg class="w-6 h-6 text-accent-400 mx-auto mb-2" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z"/>
            </svg>
            <p class="text-xs text-slate-300 font-medium">Cloud Processing</p>
            <p class="text-xs text-slate-500">รวดเร็ว ไม่ต้องติดตั้ง</p>
        </div>
    </div>
</div>
@endsection
