...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10" x-data="{ searchQuery: '', selectedType: 'all' }">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            ...
            <h1 class="text-3xl font-bold text-gray-800">คลังไฟล์ของฉัน</h1>
            <p class="text-gray-500 text-sm mt-1">ไฟล์ที่ผ่านการประมวลผลและอัปโหลดทั้งหมด พร้อมระยะเวลาเก็บรักษาตามแผนของคุณ</p>
        </div>
    ...
    <div class="glass rounded-2xl p-4 border border-gray-100 mb-6 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div class="relative w-full md:w-80">
            ...
            <input type="text" x-model="searchQuery" placeholder="ค้นหาตามชื่อไฟล์..."
                   class="w-full bg-gray-50 border border-gray-200 text-gray-800 placeholder-slate-500 rounded-xl pl-10 pr-4 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/50">
        </div>
        ...
        <div class="flex items-center gap-2 overflow-x-auto w-full md:w-auto pb-1 md:pb-0">
            <button @click="selectedType = 'all'"
                    ...
                    :class="selectedType === 'all' ? 'bg-brand-500 text-white shadow-lg shadow-brand-500/20' : 'text-gray-500 hover:text-gray-800 bg-gray-50'">
                ทั้งหมด ({{ $files->total() }})
            </button>
            <button @click="selectedType = 'pdf'"
                    class="px-3.5 py-1.5 rounded-xl text-xs font-medium transition-all"
                    :class="selectedType === 'pdf' ? 'bg-red-500 text-white' : 'text-gray-500 hover:text-gray-800 bg-gray-50'">
                PDF
            </button>
            <button @click="selectedType = 'docx'"
                    class="px-3.5 py-1.5 rounded-xl text-xs font-medium transition-all"
                    :class="selectedType === 'docx' ? 'bg-blue-500 text-white' : 'text-gray-500 hover:text-gray-800 bg-gray-50'">
                Word
            </button>
            <button @click="selectedType = 'image'"
                    class="px-3.5 py-1.5 rounded-xl text-xs font-medium transition-all"
                    :class="selectedType === 'image' ? 'bg-emerald-500 text-white' : 'text-gray-500 hover:text-gray-800 bg-gray-50'">
                รูปภาพ
            </button>
        </div>
    </div>
    ...
    <div class="glass rounded-2xl border border-gray-100 overflow-hidden">
        ...
        <div class="py-20 text-center">
            ...
            <h3 class="text-gray-800 font-semibold text-lg mb-1">ยังไม่มีไฟล์ในคลังของคุณ</h3>
            <p class="text-gray-500 text-sm max-w-sm mx-auto mb-6">ไฟล์ที่คุณทำการแปลงหรืออัปโหลดจะปรากฏที่นี่โดยอัตโนมัติ</p>
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left text-sm text-gray-600">
                <thead class="bg-white/[0.02] border-b border-gray-100 text-xs font-semibold text-gray-500">
                    <tr>
                    ...
                    <tr class="hover:bg-white/[0.02] transition-colors"
                            ...
                            <div class="flex items-center gap-3">
                                <div class="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0
                                    {{ match($typeGroup) {
                                        ...
                                        default => 'bg-slate-500/10 text-gray-500 border border-slate-500/20'
                                    } }}">
                                    <span class="text-[10px] font-bold uppercase">{{ substr($ext ?: 'FILE', 0, 4) }}</span>
                                </div>
                                <div class="min-w-0 max-w-xs md:max-w-md">
                                    <p class="font-medium text-gray-800 truncate">{{ $file->original_name }}</p>
                        ...
                        <td class="py-4 px-4 whitespace-nowrap">
                            @if($file->expires_at)
                                ...
                                @endif
                            @else
                            <span class="text-xs text-gray-400">ไม่มีกำหนด</span>
                            @endif
                        </td>
                        <td class="py-4 px-6 text-right whitespace-nowrap">
                            <div class="flex items-center justify-end gap-2">
                                ...
                                <form action="{{ route('dashboard.files.delete', $file) }}" method="POST"
                                    ...
                                    @method('DELETE')
                                    <button type="submit"
                                            class="p-2 rounded-lg bg-gray-50 hover:bg-error-500/20 text-gray-500 hover:text-error-400 transition-all"
                                            title="ลบไฟล์">
                                        <svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke-width="2" stroke="currentColor">
