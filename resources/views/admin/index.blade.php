...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <div class="flex items-center gap-3">
                <h1 class="text-3xl font-extrabold text-gray-800">แผงควบคุมระบบ (Admin)</h1>
                <span class="badge-premium text-xs">System Admin</span>
            </div>
            <p class="text-gray-500 text-sm mt-1">ภาพรวมสถิติการใช้งาน ผู้ใช้ทั้งหมด การสมัครสมาชิก และสถานะคิวงาน PDF</p>
        </div>
        <div class="flex items-center gap-3">
    ...
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        ...
        <div class="glass rounded-2xl p-5 border border-gray-100">
            ...
            <p class="text-3xl font-bold text-gray-800 mb-1">{{ number_format($activeSubscriptions) }}</p>
            <p class="text-xs text-gray-500">Pro & Business Active</p>
        </div>
        ...
        <div class="glass rounded-2xl p-5 border border-gray-100">
            ...
            <p class="text-3xl font-bold text-gray-800 mb-1">{{ number_format($totalJobs) }}</p>
        ...
        <div class="glass rounded-2xl p-5 border border-gray-100">
            ...
            @endphp
            <p class="text-3xl font-bold text-gray-800 mb-1">{{ $storageStr }}</p>
            <p class="text-xs text-gray-500">{{ number_format($totalFiles) }} ไฟล์ในระบบ</p>
        </div>
    </div>
    ...
    <div class="grid lg:grid-cols-3 gap-8 mb-8">
        ...
        <div class="glass rounded-2xl p-6 border border-gray-100">
            ...
            <div class="space-y-3">
                ...
                @empty
                <p class="text-gray-400 text-xs text-center py-6">ยังไม่มีประวัติการใช้งานเครื่องมือ</p>
                @endforelse
            </div>
        </div>
            ...
            <div class="space-y-4">
                ...
                <div class="glass-light p-3.5 rounded-xl border border-white/[0.04] flex items-center justify-between">
                    ...
                    <div class="text-right">
                        <span class="text-xl font-bold text-gray-800">{{ number_format($plan->subscriptions_count) }}</span>
                ...
                <div class="flex items-center justify-between text-xs py-1.5 border-b border-white/[0.04] last:border-0">
                    <div class="min-w-0 flex items-center gap-2">
                        ...
                        <div class="truncate">
                            ...
                            <p class="text-gray-400 truncate">{{ $u->email }}</p>
                        </div>
                    </div>
                    <span class="badge-free text-[10px]">{{ $u->getActivePlan()->name }}</span>
                </div>
                @empty
                <p class="text-gray-400 text-xs text-center py-6">ไม่มีข้อมูล</p>
                @endforelse
            </div>
        </div>
    </div>
    ...
    <div class="glass rounded-2xl border border-gray-100 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
            ...
            <span class="text-xs text-gray-500">15 รายการล่าสุด</span>
        </div>

        <div class="overflow-x-auto">
            <table class="w-full text-left text-xs text-gray-600">
                <thead class="bg-white/[0.02] border-b border-gray-100 text-gray-500 font-semibold">
                    <tr>
                ...
                <tbody class="divide-y divide-white/[0.04]">
                    ...
                    <tr class="hover:bg-white/[0.02] transition-colors">
                        <td class="py-3 px-6 font-mono text-gray-500">
                            {{ substr($job->id, 0, 8) }}...
                        </td>
                        <td class="py-3 px-4">
                            @if($job->user)
                            <span class="font-medium text-gray-800">{{ $job->user->name }}</span>
                            @else
                            <span class="text-gray-400">Guest (Session)</span>
                            @endif
                        </td>
                        <td class="py-3 px-4">
                            <span class="px-2 py-0.5 rounded-md bg-gray-50 text-gray-600 font-mono">{{ $job->tool_name }}</span>
                        </td>
                        <td class="py-3 px-4">
                            @if($job->status === 'done')
                        ...
                        <td class="py-3 px-6 text-right text-gray-400 whitespace-nowrap">
                            {{ $job->created_at->diffForHumans() }}
                        </td>
                    </tr>
                    @empty
                    <tr>
                        <td colspan="6" class="text-center py-8 text-gray-400">ยังไม่มีงานในระบบ</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
