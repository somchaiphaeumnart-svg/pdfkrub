...
<div class="min-h-[calc(100vh-4rem)] flex items-center justify-center py-16 px-4">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold text-white">สมัครสมาชิกฟรี</h1>
            <p class="text-slate-400 mt-1 text-sm">เริ่มจัดการ PDF ได้ทันที ไม่ต้องใส่บัตรเครดิต</p>
        </div>
        ...
        <div class="glass rounded-2xl p-8 border border-white/[0.08]">
            ...
            <form method="POST" action="{{ route('register.post') }}" class="space-y-4">
                ...
                {{-- Name --}}
                <div>
                    <label for="name" class="block text-sm font-medium text-slate-300 mb-1.5">ชื่อ-นามสกุล</label>
                    <input id="name" type="text" name="name" value="{{ old('name') }}" required
                           autocomplete="name" placeholder="สมชาย ใจดี"
                           class="w-full bg-white/5 border {{ $errors->has('name') ? 'border-error-500' : 'border-white/10' }} text-white placeholder-slate-500 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition-all">
                    @error('name')
                ...
                <div x-data="{ show: false }">
                    ...
                    <div class="relative">
                        <input id="password" :type="show ? 'text' : 'password'" name="password" required
                               autocomplete="new-password" placeholder="อย่างน้อย 8 ตัวอักษร"
                ...
                {{-- Confirm Password --}}
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-slate-300 mb-1.5">ยืนยันรหัสผ่าน</label>
                    <input id="password_confirmation" type="password" name="password_confirmation" required
                           autocomplete="new-password" placeholder="••••••••"
                           class="w-full bg-white/5 border border-white/10 text-white placeholder-slate-500 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition-all">
                </div>

                {{-- Terms --}}
                <div class="flex items-start gap-2">
                    <input id="terms" name="terms" type="checkbox" required
                           class="w-4 h-4 rounded border-white/20 bg-white/5 text-brand-500 focus:ring-brand-500/50 mt-0.5 flex-shrink-0">
                ...
                <button type="submit" class="w-full btn-primary py-3.5 rounded-xl text-sm mt-2">
                    สมัครสมาชิกฟรี
                </button>
            </form>
        </div>
