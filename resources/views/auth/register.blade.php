...
<div class="min-h-[calc(100vh-4rem)] flex items-center justify-center py-16 px-4">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold text-gray-800">สมัครสมาชิกฟรี</h1>
        ...
        <div class="bg-white border border-gray-100 shadow-sm rounded-2xl p-8 border border-gray-100">
            ...
            <form method="POST" action="{{ route('register.post') }}" class="space-y-4">
                ...
                {{-- Name --}}
                <div>
                    <label for="name" class="block text-sm font-medium text-gray-600 mb-1.5">ชื่อ-นามสกุล</label>
                    <input id="name" type="text" name="name" value="{{ old('name') }}" required
                           autocomplete="name" placeholder="สมชาย ใจดี"
                           class="w-full bg-gray-50 border {{ $errors->has('name') ? 'border-error-500' : 'border-gray-200' }} text-gray-800 placeholder-slate-500 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition-all">
                    @error('name')
                ...
                <div x-data="{ show: false }">
                    <label for="password" class="block text-sm font-medium text-gray-600 mb-1.5">รหัสผ่าน</label>
                    <div class="relative">
                ...
                {{-- Confirm Password --}}
                <div>
                    <label for="password_confirmation" class="block text-sm font-medium text-gray-600 mb-1.5">ยืนยันรหัสผ่าน</label>
                    <input id="password_confirmation" type="password" name="password_confirmation" required
                           autocomplete="new-password" placeholder="••••••••"
                           class="w-full bg-gray-50 border border-gray-200 text-gray-800 placeholder-slate-500 rounded-xl px-4 py-3 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500/50 focus:border-brand-500 transition-all">
                </div>
