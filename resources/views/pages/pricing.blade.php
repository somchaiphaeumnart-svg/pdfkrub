...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
    ...
    <div class="text-center mb-16">
        ...
        <h1 class="text-4xl sm:text-5xl font-bold text-white mb-4">แผนราคาที่ <span class="text-gradient">ครูเลือกได้</span></h1>
        <p class="text-slate-400 max-w-xl mx-auto">เริ่มต้นฟรี ไม่ต้องใส่บัตรเครดิต · รองรับ PDPA · ประมวลผลในประเทศไทย</p>

        {{-- Billing toggle --}}
        <div class="mt-8 inline-flex items-center glass rounded-full p-1.5 border border-white/[0.08]" x-data="{ yearly: false }">
            <button @click="yearly = false"
                    class="px-5 py-2 text-sm rounded-full transition-all"
                    :class="!yearly ? 'bg-brand-600 text-white font-medium' : 'text-slate-400 hover:text-white'">
                รายเดือน
            </button>
    ...
    <div class="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto mb-12" x-data="{ yearly: false }">
        ...
        <div class="glass rounded-2xl p-8 border card-hover relative flex flex-col
                    {{ $isFeatured ? 'border-brand-500/50 glow-blue' : ($isTeacher ? 'border-success-500/30' : 'border-white/[0.06]') }}">

            @if($isFeatured)
            <div class="absolute -top-4 left-1/2 -translate-x-1/2">
                <span class="bg-gradient-to-r from-brand-600 to-brand-400 text-white text-xs font-bold px-4 py-1.5 rounded-full shadow-lg">⭐ แนะนำ</span>
            </div>
            @endif
            @if($isTeacher)
            <div class="absolute -top-4 left-1/2 -translate-x-1/2">
                <span class="bg-gradient-to-r from-success-600 to-success-400 text-white text-xs font-bold px-4 py-1.5 rounded-full shadow-lg">🎓 สำหรับครู</span>
            </div>
            @endif
            ...
            <div class="mb-6">
                <div class="flex items-center gap-2 mb-3">
                    ...
                    <h2 class="text-xl font-bold text-white">{{ $plan->display_name_th ?? $plan->display_name }}</h2>
                </div>
                ...
                <div x-show="!yearly">
                    ...
                    @if($isTeacher)
                    <p class="text-xs text-slate-500 mt-1">*จ่ายรายปี ฿{{ number_format($plan->price_yearly) }} เท่านั้น</p>
                    @endif
                    @else
                ...
                <div x-show="yearly" style="display:none">
                    ...
                    <div class="flex items-baseline gap-1">
                        <span class="text-4xl font-bold text-white">฿{{ number_format(round($plan->price_yearly / 12)) }}</span>
                        <span class="text-slate-400 text-sm">/เดือน</span>
                    </div>
            ...
            <ul class="space-y-3 flex-1 mb-8">
                ...
                <li class="flex items-center gap-2.5 text-sm {{ $feat['check'] ? 'text-slate-300' : 'text-slate-600' }}">
                    @if($feat['check'])
            ...
            @else
                <a href="{{ route('register') }}"
                   class="w-full block text-center {{ $isFeatured ? 'btn-primary' : ($isTeacher ? 'bg-success-600 hover:bg-success-500 text-white font-semibold transition-colors' : 'btn-ghost') }} rounded-xl py-3.5 text-sm">
    ...
    <div class="max-w-5xl mx-auto mb-16">
        <div class="text-center mb-8">
            <h2 class="text-2xl font-bold text-white mb-2">แผนสำหรับ <span class="text-gradient">โรงเรียนและองค์กร</span></h2>
            <p class="text-slate-400 text-sm">ใช้งานร่วมกันทั้งโรงเรียน บริหารสิทธิ์ครูได้ รองรับ PDPA ระดับสถาบัน</p>
        </div>
        ...
        <div class="grid md:grid-cols-2 gap-6" x-data="{ yearly: true }">
            ...
            <div class="glass rounded-2xl p-8 border {{ $isSchool ? 'border-brand-500/30' : 'border-white/[0.06]' }} card-hover relative flex flex-col">
                @if($isSchool)
                <div class="absolute -top-3 right-6">
                    <span class="bg-brand-600/80 backdrop-blur text-white text-xs font-bold px-3 py-1 rounded-full">🏫 โรงเรียน</span>
                </div>
                @endif
                ...
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <h3 class="text-xl font-bold text-white">{{ $plan->display_name_th }}</h3>
                        <p class="text-xs text-slate-500">{{ $isSchool ? 'ครูสูงสุด ' . $plan->max_team_members . ' คน' : 'สมาชิกสูงสุด ' . $plan->max_team_members . ' คน' }}</p>
                    </div>
                </div>

                <div class="mb-6">
                    <div class="flex items-baseline gap-1">
                        <span class="text-3xl font-bold text-white">฿{{ number_format($plan->price_yearly) }}</span>
                        <span class="text-slate-400 text-sm">/ปี</span>
                    </div>
                    <p class="text-xs text-slate-500 mt-1">≈ ฿{{ number_format(round($plan->price_yearly / 12)) }}/เดือน</p>
                    @if($isSchool)
    ...
    <div class="max-w-5xl mx-auto mb-16">
        <div class="glass rounded-2xl p-6 border border-success-500/20 bg-gradient-to-r from-success-500/5 to-brand-500/5">
            <div class="flex flex-col md:flex-row items-center gap-4 text-center md:text-left">
                ...
                <div class="flex-1">
                    <h3 class="text-white font-bold mb-1">ปลอดภัย รองรับ PDPA · ประมวลผลในประเทศไทย</h3>
                    <p class="text-slate-400 text-sm">เอกสารทุกไฟล์ประมวลผลบนเซิร์ฟเวอร์ไทย ไม่ส่งออกนอกประเทศ ลบอัตโนมัติตามแผน เหมาะสำหรับเอกสารสำคัญ เช่น เลขบัตรประชาชน โปรไฟล์นักเรียน</p>
                </div>
    ...
    <div class="max-w-3xl mx-auto">
        <h2 class="text-2xl font-bold text-white text-center mb-10">คำถามที่พบบ่อย</h2>
        ...
        <div class="space-y-4" x-data="{ open: null }">
            ...
            <div class="glass rounded-xl border border-white/[0.06] overflow-hidden">
                ...
                <div x-show="open === {{ $i }}"
                     ...
                     class="px-6 pb-5 text-sm text-slate-400 leading-relaxed border-t border-white/[0.06] pt-4">
                    {{ $faq['a'] }}
                </div>
            </div>
            @endforeach
